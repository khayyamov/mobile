...
func (g *ClassGen) GenC() {
	...
	for _, cls := range g.classes {
		g.Printf("clazz = libv2ray_seq_find_class(%q);\n", strings.Replace(cls.FindName, ".", "/", -1))
		...
		for _, fs := range cls.Funcs {
			for _, f := range fs.Funcs {
				...
				if f.Constructor {
					g.Printf("libv2ray_seq_get_method_id(clazz, \"<init>\", %q);\n", f.Desc)
				} else {
					g.Printf("libv2ray_seq_get_static_method_id(clazz, %q, %q);\n", f.Name, f.Desc)
				}
			}
		}
		for _, fs := range cls.AllMethods {
			for _, f := range fs.Funcs {
				if g.isFuncSupported(f) {
					g.Printf("m_%s_%s = libv2ray_seq_get_method_id(clazz, %q, %q);\n", cls.JNIName, f.JNIName, f.Name, f.Desc)
					if _, ok := g.goClsMap[cls.Name]; ok {
						g.Printf("sm_%s_%s = libv2ray_seq_get_method_id(sclass_%s, %q, %q);\n", cls.JNIName, f.JNIName, cls.JNIName, f.Name, f.Desc)
					}
				}
			}
		}
...
func (g *ClassGen) genCMethodBody(cls *java.Class, f *java.Func, virtual bool) {
	...
	g.Printf("jobject _this = libv2ray_seq_from_refnum(env, this, NULL, NULL);\n")
	...
	g.Printf(");\n")
	g.Printf("jobject _exc = libv2ray_seq_get_exception(env);\n")
	g.Printf("int32_t _exc_ref = libv2ray_seq_to_refnum(env, _exc);\n")
	if f.Ret != nil {
...
func (g *ClassGen) genGo(cls *java.Class) {
	...
	g.Printf("clazz := C.libv2ray_seq_find_class(cls)\n")
	...
	g.Printf("if C.libv2ray_seq_isinstanceof(ref, class_%s) != 1 {\n", cls.JNIName)
	...
	if goName, ok := g.goClsMap[cls.Name]; ok {
		...
		g.Indent()
		g.Printf("goRefnum := C.libv2ray_seq_unwrap(C.jint(p.Bind_proxy_refnum__()))\n")
...
func (g *ClassGen) genJavaToC(v string, t *java.Type) {
	...
	case java.Array:
		...
		g.Printf("nbyteslice _%s = libv2ray_seq_from_java_bytearray(env, %s, 1);\n", v, v)
	case java.Object:
		g.Printf("jint _%s = libv2ray_seq_to_refnum(env, %s);\n", v, v)
	default:
		panic("invalid kind")
	}
}
...
func (g *ClassGen) genCToJava(v string, t *java.Type) {
	...
	case java.Int, java.Short, java.Char, java.Byte, java.Long, java.Float, java.Double, java.Boolean:
		g.Printf("%s _%s = %s;\n", t.JNIType(), v, v)
	case java.String:
		g.Printf("jstring _%s = libv2ray_seq_to_java_string(env, %s);\n", v, v)
	case java.Array:
		if t.Elem.Kind != java.Byte {
			panic("unsupported array type")
		}
		g.Printf("jbyteArray _%s = libv2ray_seq_to_java_bytearray(env, %s, 0);\n", v, v)
	case java.Object:
		g.Printf("jobject _%s = libv2ray_seq_from_refnum(env, %s, NULL, NULL);\n", v, v)
	default:
		panic("invalid kind")
	}
}
