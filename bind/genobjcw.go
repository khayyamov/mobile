...
func (g *ObjcWrapper) genCFuncBody(n *objc.Named, f *objc.Func, super bool) {
	...
	g.Indent()
	if !f.Static {
		g.Printf("%s _this = libbox_seq_from_refnum(this).obj;\n", n.ObjcType())
	}
...
func (g *ObjcWrapper) genCToObjC(name string, t *objc.Type, mode varMode) {
	switch t.Kind {
	case objc.String:
		g.Printf("NSString *_%s = libbox_seq_to_objc_string(%s);\n", name, name)
	case objc.Bool:
		g.Printf("BOOL _%s = %s ? YES : NO;\n", name, name)
	case objc.Data:
		g.Printf("NSData *_%s = libbox_seq_to_objc_bytearray(%s, %d);\n", name, name, toCFlag(mode == modeRetained))
...
func (g *ObjcWrapper) genObjCToC(name string, t *objc.Type, mode varMode) {
	...
	case objc.Protocol, objc.Class:
		g.Printf("int _%s = libbox_seq_to_refnum(%s);\n", name, name)
	default:
		panic("invalid kind")
	}
}
