...
func (g *JavaGen) genJavaToC(varName string, t types.Type, mode varMode) {
	...
	case *types.Slice:
		...
		case *types.Basic:
			switch e.Kind() {
			case types.Uint8: // Byte.
				g.Printf("nbyteslice _%s = wireguardwarp_seq_from_java_bytearray(env, %s, %d);\n", varName, varName, toCFlag(mode == modeRetained))
			default:
	...
	case *types.Named:
		...
		case *types.Interface:
			g.Printf("int32_t _%s = wireguardwarp_seq_to_refnum(env, %s);\n", varName, varName)
		default:
...
func (g *JavaGen) genCToJava(toName, fromName string, t types.Type, mode varMode) {
	...
	case *types.Basic:
		switch t.Kind() {
		case types.String:
			g.Printf("jstring %s = wireguardwarp_seq_to_java_string(env, %s);\n", toName, fromName)
		case types.Bool:
	...
	case *types.Slice:
		...
		case *types.Basic:
			switch e.Kind() {
			case types.Uint8: // Byte.
				g.Printf("jbyteArray %s = wireguardwarp_seq_to_java_bytearray(env, %s, %d);\n", toName, fromName, toCFlag(mode == modeRetained))
			default:
				g.errorf("unsupported type: %s", t)
			}
		default:
...
func (g *JavaGen) genFromRefnum(toName, fromName string, t types.Type, o *types.TypeName) {
	...
	p := pkgPrefix(oPkg)
	g.Printf("jobject %s = wireguardwarp_seq_from_refnum(env, %s, ", toName, fromName)
	if isJava {
		g.Printf("NULL, NULL")
	} else {
...
func (g *JavaGen) genJNIField(o *types.TypeName, f *types.Var) {
	...
	g.Indent()
	g.Printf("int32_t o = wireguardwarp_seq_to_refnum_go(env, this);\n")
...
func (g *JavaGen) genJNIConstructor(f *types.Func, sName string) {
	...
	if res.Len() == 2 {
		...
		g.Printf("wireguardwarp_seq_maybe_throw_exception(env, _err);\n")
	}
	g.Printf("return refnum;\n")
	g.Outdent()
	g.Printf("}\n\n")
...
func (g *JavaGen) genJNIFuncBody(o *types.Func, sName string, jm *java.Func, isjava bool) {
	...
	res := sig.Results()
	if sName != "" {
		g.Printf("int32_t o = wireguardwarp_seq_to_refnum_go(env, __this__);\n")
	}
	params := sig.Params()
	first := 0
	if isjava && params.Len() > 0 && params.At(0).Name() == "this" {
		...
		first = 1
		g.Printf("int32_t _%s = wireguardwarp_seq_to_refnum(env, __this__);\n", g.paramName(params, 0))
	}
	for i := first; i < params.Len(); i++ {
		name := g.paramName(params, i)
		g.genJavaToC(name, params.At(i).Type(), modeTransient)
	...
	for i := res.Len() - 1; i >= 0; i-- {
		...
		if !isErrorType(t) {
			...
			g.Printf("wireguardwarp_seq_maybe_throw_exception(env, _r%d);\n", i)
		}
	}
}
		...
		case *types.Basic:
			...
			case types.Uint8: // Byte.
				if mode == modeTransient {
					g.Printf("wireguardwarp_seq_release_byte_array(env, %s, _%s.ptr);\n", varName, varName)
				}
			}
		}
	}
...
func (g *JavaGen) genMethodInterfaceProxy(oName string, m *types.Func) {
	...
	g.Printf("jobject o = wireguardwarp_seq_from_refnum(env, refnum, proxy_class_%s_%s, proxy_class_%s_%s_cons);\n", g.pkgPrefix, oName, g.pkgPrefix, oName)
	for i := 0; i < params.Len(); i++ {
	...
	if res.Len() > 0 {
		...
		if res.Len() == 2 || isErrorType(t) {
			g.Printf("jobject exc = wireguardwarp_seq_get_exception(env);\n")
