...
func (g *ObjcGen) genGetter(oName string, f *types.Var) {
	...
	g.Indent()
	g.Printf("int32_t refnum = wireguardwarp_seq_go_to_refnum(self._ref);\n")
...
func (g *ObjcGen) genWrite(varName string, t types.Type, mode varMode) {
	...
	case *types.Basic:
		switch t.Kind() {
		case types.String:
			g.Printf("nstring _%s = wireguardwarp_seq_from_objc_string(%s);\n", varName, varName)
		default:
	...
	case *types.Slice:
		...
		case *types.Basic:
			...
			case types.Uint8: // Byte.
				g.Printf("nbyteslice _%s = wireguardwarp_seq_from_objc_bytearray(%s, %d);\n", varName, varName, toCFlag(mode == modeRetained))
			default:
...
func (g *ObjcGen) genRefWrite(varName string) {
	...
	g.Printf("_%s = wireguardwarp_seq_go_to_refnum(%s_proxy._ref);\n", varName, varName)
	g.Outdent()
	g.Printf("} else {\n")
	g.Indent()
	g.Printf("_%s = wireguardwarp_seq_to_refnum(%s);\n", varName, varName)
	g.Outdent()
	g.Printf("}\n")
}
...
func (g *ObjcGen) genRefRead(toName, fromName string, t types.Type) {
	...
	g.Printf("%s* %s = nil;\n", ptype, toName)
	g.Printf("GoSeqRef* %s_ref = wireguardwarp_seq_from_refnum(%s);\n", toName, fromName)
...
func (g *ObjcGen) genRead(toName, fromName string, t types.Type, mode varMode) {
	...
	case *types.Basic:
		switch t.Kind() {
		case types.String:
			g.Printf("NSString *%s = wireguardwarp_seq_to_objc_string(%s);\n", toName, fromName)
		case types.Bool:
	...
	case *types.Slice:
		...
		case *types.Basic:
			switch e.Kind() {
			case types.Uint8: // Byte.
				g.Printf("NSData *%s = wireguardwarp_seq_to_objc_bytearray(%s, %d);\n", toName, fromName, toCFlag(mode == modeRetained))
			default:
				g.errorf("unsupported type: %s", t)
			}
		default:
...
func (g *ObjcGen) genFunc(s *funcSummary, objName string) {
	...
	if objName != "" {
		...
		if s.hasself {
			skip = 1
			g.Printf("int32_t _self = wireguardwarp_seq_to_refnum(self);\n")
		}
	}
...
func (g *ObjcGen) genInterfaceMethodProxy(obj *types.TypeName, m *types.Func) {
	...
	g.Indent()
	g.Printf("%s* o = wireguardwarp_seq_objc_from_refnum(refnum);\n", g.refTypeBase(obj.Type()))
	for _, p := range s.params {
		...
		case *types.Basic:
			...
			case types.Uint8: // Byte.
				if mode == modeTransient {
					// If the argument was not mutable, wireguardwarp_seq_from_objc_bytearray created a copy.
					// Free it here.
...
func (g *ObjcGen) genStructM(obj *types.TypeName, t *types.Struct) {
	...
	if oinf != nil || len(cons) == 0 {
		...
		g.Printf("if (self) {\n")
		g.Indent()
		g.Printf("__ref = wireguardwarp_seq_from_refnum(new_%s_%s());\n", g.pkgPrefix, obj.Name())
		g.Outdent()
		g.Printf("}\n")
		g.Printf("return self;\n")
		g.Outdent()
...
func (g *ObjcGen) genInitM(obj *types.TypeName, f *types.Func) {
	...
	if len(s.retParams) == 2 {
		...
		g.Printf("GoSeqRef *_err = wireguardwarp_seq_from_refnum(res.r1);\n")
	}
	g.Printf("__ref = wireguardwarp_seq_from_refnum(refnum);\n")
	if len(s.retParams) == 2 {
